...
const DEFAULT_MODEL = process.env.AZZCO_NOTIFY_MODEL || "together/openai/gpt-oss-20b";
...
function sendMessage(channel, target, message) {
  return run("openclaw", ["message", "send", "--channel", channel, "--target", target, "--message", message], 90000);
}

function queueWhatsappCron(message, priority) {
  const name = `AZZCO owner notify ${priority} ${Date.now()}`;
  const prompt = [
