...
const contextFiles = {
  ...
  deterministic_schedule: path.join(RUNNER, "azzco-openclaw-runner.cron"),
  deterministic_schedule_compat: "/etc/cron.d/azzco-openclaw-runner"
};
...
console.log(JSON.stringify({
  ...
  server: "hostinger_openclaw",
  purpose: "deterministic_ops_status_for_owner_reports",
  canonicalScheduler: {
    kind: "host_linux_cron",
    hostPath: "/etc/cron.d/azzco-openclaw-runner",
    workspaceMirror: path.join(RUNNER, "azzco-openclaw-runner.cron"),
    containerCompatMirror: "/etc/cron.d/azzco-openclaw-runner",
    note: "OpenClaw hard-work agent-crons are intentionally disabled. Do not treat that as missing jobs."
  },
  rules: {
