...
function categoryFor(file, text) {
  ...
  if (/february|march|april|relev[eé]s? de compte|bank|qonto statement|_2026\.pdf/.test(rel) && /solde au|transactions|iban|qonto/.test(sample)) return "bank_statement";
  if (/facture|invoice|receipt|re[cç]u|bill/.test(rel)) return "invoice_or_receipt";
...
function isNoiseLine(line) {
  return /^(date de valeur|transactions|d[ée]bit|cr[ée]dit|entr[ée]es|sorties|solde|iban|bic)$/i.test(line.trim());
}
...
function sanitizeTransactionBlock(block) {
  ...
  for (const rawLine of block) {
    ...
    if (/^azz&co labs\b/i.test(line)) continue;
...
function parseStatementSummary(doc) {
  ...
  const entries = text.match(/Entr[ée]es\s*\n\s*(\+\s*[0-9][^\n]*EUR)/i);
...
function invoiceCandidate(doc) {
  ...
  const vendor = firstLines(doc.text || doc.name, 8).find((line) => /[A-Za-zÀ-ÿ]/.test(line)) || doc.name;
  return {
    id: doc.id,
    name: doc.name,
    path: doc.path,
...
function normalizeTokens(value) {
  return String(value || "")
    ...
    .replace(/[^a-z0-9à-ÿ]+/g, " ")
    .split(/\s+/)
    .filter((t) => t.length >= 4 && !["carte", "qonto", "eur"].includes(t));
}
...
const statementValidation = statementSummaries.map((summary) => {
  ...
  const DELTA_TOLERANCE = 1.50; // Qonto micro-fees / rounding. Deltas above this need investigation.
...
// Preserve Qonto API source data — do not overwrite with PDF-parsed data if Qonto API snapshot is current and ok.
const existingValidationPath = path.join(OUT_DIR, "bank_statement_validation.json");
let preserveQontoSource = false;
try {
  const existing = JSON.parse(fs.readFileSync(existingValidationPath, "utf8"));
  preserveQontoSource = existing && existing.ok === true && typeof existing.method === "string" && existing.method.startsWith("qonto_api");
} catch {}

if (!preserveQontoSource) {
