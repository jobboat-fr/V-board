...
const JOBS_PATH = process.env.VBOARD_JOBS_PATH || "/workspace/.agent_runtime/cron/jobs.json";
...
async function main() {
  const routeTests = [
    ...
    {
      name: "morning_brief_back_office",
      expect: "council_required",
      payload: {
    ...
    {
      name: "legal_finance_sentinel_back_office",
      expect: "council_required",
      payload: {
    ...
    {
      name: "invoice_reconciliation_back_office",
      expect: "council_required",
      payload: {
        category: "invoice_reconciliation",
        urgency: "P2",
        owner: true,
        prompt: "Reconcile bank transactions, receipts, invoices and bank API evidence."
      }
    },
    {
      name: "cto_audit_back_office",
      expect: "council_required",
      payload: {
        category: "cto_audit",
        urgency: "P2",
    ...
    {
      name: "approval_queue_back_office",
      expect: "council_required",
      payload: {
    ...
    {
      name: "non_owner_restricted_back_office",
      expect: "council_required",
      payload: {
  ...
  const cronRows = Array.isArray(jobs)
    ? jobs.filter((job) => job.enabled && job.name.startsWith("VBOARD")).map((job) => {
      const message = job.payload?.message || "";
      return {
        name: job.name,
        has_route_gate: message.includes("vboard_route_policy.js") || message.includes("vboard_route_or_bridge.js"),
        has_task_division: message.includes("front_desk_back_office_task_division.md"),
  ...
  const channels = run("agent_runtime", ["channels", "status", "--probe"], null, 90000);
  const status = run("agent_runtime", ["status"], null, 90000);
  const activeModel = run("agent_runtime", ["models", "status", "--plain"], null, 90000);
  const fallbacks = run("agent_runtime", ["models", "fallbacks", "list"], null, 90000);
  const backOffice = await httpGetJson(process.env.VBOARD_COUNCIL_HEALTH_URL || "http://127.0.0.1:8787/health");
  ...
  if (!backOffice.ok) failures.push("back_office_health_failed");
  ...
  console.log(JSON.stringify({
    ...
    runner,
    cron: {
      total_vboard_enabled: cronRows.length,
