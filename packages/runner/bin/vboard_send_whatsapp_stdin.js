...
const deliveryMode = (process.env.VBOARD_WHATSAPP_DELIVERY_MODE || "relay").toLowerCase();
if (!target) {
  console.error("TARGET is required");
  process.exit(2);
}

const body = fs.readFileSync(0, "utf8").trim() || "VBOARD runner produced an empty report.";
const max = 3200;
const chunks = [];
for (let i = 0; i < body.length; i += max) chunks.push(body.slice(i, i + max));

function sendDirectWhatsApp(message) {
  return spawnSync("agent_runtime", [
    "message",
    "send",
    "--channel",
    "whatsapp",
...
function relayWhatsApp(message) {
  const prompt = [
    "Courier instruction: relay exactly the text between BEGIN_VBOARD_REPORT and END_VBOARD_REPORT to the owner on WhatsApp.",
