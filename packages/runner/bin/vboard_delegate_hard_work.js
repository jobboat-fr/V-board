...
const collect = run(path.join(ROOT, "bin/vboard_morning_collect.sh"), [], "", 120000);
...
function buildCategoryEvidence(base) {
  ...
  if (["cto_audit","incident_watchdog","morning_brief"].includes(category)) {
    try {
      const logDir = "/root/vboard-agent_runtime-runner/logs";
...
const packet = {
  ...
  execution_contract: {
    front_desk_collects: true,
    front_desk_sends: true,
    front_desk_writes_crm: true,
...
const bridge = run("node", [path.join(ROOT, "bin/vboard_route_or_bridge.js")], JSON.stringify(packet), 360000);
