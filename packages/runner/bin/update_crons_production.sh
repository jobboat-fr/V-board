...
CORE="Production mode. Read $WORKSPACE/protocols/runner_council_task_division.md. runner is sender/communicator/CRM writer. council prepares only. Before hard analysis or any council call, build compact JSON and run node $WORKSPACE/bin/azzco_route_or_bridge.js. If route=runner_local, execute locally. If bridge_called=true, execute only explicit allowed actions. Use $WORKSPACE/bin/azzco_notify_owner.js for owner notifications when tool delivery is unavailable. Do not paste huge docs/logs/inboxes; use compact evidence."
...
edit_job 77a662dc-5ddb-4d2d-94bb-8a62b36614fc "$CORE Incident watchdog: first run node $WORKSPACE/bin/azzco_prod_doctor.js. If doctor is OK, send short all-clear. If failures are P0/P1 or ambiguous, route compact evidence to council. Avoid large logs unless owner asks."

edit_job a44c8e0d-62db-40a2-909c-28ed377f6845 "$CORE Legal finance sentinel: legal/accounting/fiscal/social/Qonto/bank/invoice/receipt risks route to council with compact evidence. runner never files, pays, signs, or exposes confidential data."
...
edit_job 2e8d6580-8ccc-4f17-b34a-8b21d14725f7 "$CORE Invoice follow-up: invoice matching, missing proofs, fiscal/accounting ambiguity, Qonto/bank evidence route to council. runner delivers owner action list."
...
edit_job 15a92bb7-938a-465d-8c0f-4e673bd849cc "$CORE Deep accounting scan: council-required weekly only. Send compact Qonto/bank/invoice evidence. runner delivers missing-proof list."
