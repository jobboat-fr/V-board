...
const titles = {
  morning_brief: "VBOARD Morning Brief",
  incident_watchdog: "VBOARD Incident Watchdog",
  cto_audit: "VBOARD CTO Platform Audit",
  lead_scout: "VBOARD Lead Scout",
  crm_pipeline: "VBOARD CRM Pipeline",
  mail_triage: "VBOARD Mail Triage",
  legal_accounting: "VBOARD Legal/Accounting Review",
  legal_finance_sentinel: "VBOARD Legal Finance Sentinel",
  invoice_reconciliation: "VBOARD Invoice Reconciliation",
  deal_desk: "VBOARD Deal Desk",
  approval_queue: "VBOARD Approval Queue",
  document_vault: "VBOARD Document Vault Audit",
  deep_legal_scan: "VBOARD Deep Legal Scan",
  deep_accounting_scan: "VBOARD Deep Accounting Scan",
  deep_risk_synthesis: "VBOARD Deep Risk Synthesis",
  weekly_board_brief: "VBOARD Weekly Board Brief",
  owner_decision_meeting: "VBOARD Owner Decision Brief"
};

function runDelegate() {
  return spawnSync("node", [path.join(ROOT, "bin/vboard_delegate_hard_work.js"), category, urgency], {
    encoding: "utf8",
    timeout: 420000,
...
function render(result) {
  ...
  const backOffice = stage.bridge || {};
  const route = stage.route || {};
  const decision = backOffice.decision || {};
  const packets = Array.isArray(backOffice.packets) ? backOffice.packets : [];
  const modelPlan = backOffice.route?.models?.plan || [];
  const budget = backOffice.route?.budget || {};
  const risks = unique([
    decision.top_risks || [],
  ...
  if (!result?.ok || !stage.ok || !backOffice.ok) {
    return renderBlocked(backOffice.error || stage.error || result?.error || "council bridge returned non-ok", JSON.stringify({ stage, backOffice }).slice(0, 900));
  }
