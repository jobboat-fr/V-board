...
GATE="Read $WORKSPACE/protocols/runner_front_desk_council_offload.md and $WORKSPACE/protocols/runner_council_task_division.md. Before any council call, build compact JSON and run node $WORKSPACE/bin/vboard_route_policy.js. If route=runner_local, handle locally on runner. If route=council_required, call $WORKSPACE/bin/vboard_council_call.sh with compact evidence only. council prepares only; runner sends, communicates, writes CRM, and reports."
...
edit_job a44c8e0d-62db-40a2-909c-28ed377f6845 "$GATE Legal finance sentinel: legal/accounting/fiscal/social/bank API/bank/invoice/receipt risks always route to council with compact evidence. runner never files, pays, signs, or exposes confidential data."
...
edit_job 2e8d6580-8ccc-4f17-b34a-8b21d14725f7 "$GATE Invoice follow-up: invoice matching, missing proofs, fiscal/accounting ambiguity, bank API/bank evidence route to council. runner delivers owner action list and transfers files only when allowed."
...
edit_job 15a92bb7-938a-465d-8c0f-4e673bd849cc "$GATE Deep accounting scan: always council-required, weekly only, compact bank API/bank/invoice evidence. runner delivers missing-proof list."
