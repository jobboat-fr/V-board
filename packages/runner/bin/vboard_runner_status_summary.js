...
const schedule = fs.existsSync(path.join(RUNNER, "vboard-agent_runtime-runner.cron"))
  ? fs.readFileSync(path.join(RUNNER, "vboard-agent_runtime-runner.cron"), "utf8").trim()
  : "schedule mirror missing";

const categories = Object.keys(labels);
const text = [
  "# VBOARD Current Scheduler Status",
  "",
  `Generated: ${new Date().toISOString()} UTC / ${parisTime(new Date().toISOString())} Europe/Paris`,
  "",
  "Source of truth:",
  "- Canonical scheduler: host Linux cron /etc/cron.d/vboard-agent_runtime-runner",
  `- Workspace mirror: ${ROOT}/ops/runner/vboard-agent_runtime-runner.cron`,
  "- agent runtime hard-work agent-crons are intentionally disabled. Do not report them as missing.",
