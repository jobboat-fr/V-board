...
const STATE_DIR = process.env.AGENT_RUNTIME_STATE_DIR || "/data/.agent_runtime";
const OWNER_WHATSAPP = process.env.VBOARD_OWNER_WHATSAPP || "";
const OWNER_TELEGRAM = process.env.VBOARD_OWNER_TELEGRAM || "";
const DAILY_TOKEN_WARN = Number(process.env.VBOARD_DAILY_TOKEN_WARN || 750000);
const SINGLE_RUN_INPUT_WARN = Number(process.env.VBOARD_SINGLE_RUN_INPUT_WARN || 200000);
const SMALL_MODEL_INPUT_WARN = Number(process.env.VBOARD_SMALL_MODEL_INPUT_WARN || 100000);
const LOOKBACK_MS = Number(process.env.VBOARD_GUARDRAIL_LOOKBACK_MS || 24 * 60 * 60 * 1000);
...
function buildReport() {
  ...
  return [
    `VBOARD Deterministic Cost Guardrail`,
    `Window: last 24h`,
    `Status: ${status}`,
    ``,
    `Evidence [EMP]:`,
