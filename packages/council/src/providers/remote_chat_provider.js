...
function stripProviderPrefix(model) {
  return String(model || "").replace(/^remote\//, "");
}
...
class RemoteChatProvider {
  constructor({
    token = process.env.LLM_API_TOKEN || process.env.LLM_API_TOKEN || process.env.LLM_API_TOKEN || "",
  ...
  promptFor({ role, route, request, workflow }) {
    return [
      "You are one specialist worker in the V-Board back-office hard-work council.",
  ...
  async runRole(args) {
    ...
    if (!this.available()) {
      ...
      return {
        ...packet,
        provider_error: "remote LLM provider token is not configured",
    ...
    const body = {
      ...
      temperature: 0.2,
      max_tokens: Number(process.env.VBOARD_LLM_CHAT_MAX_TOKENS || 1400),
    ...
    try {
      ...
      response = await fetch(this.baseUrl, {
        ...
        signal: AbortSignal.timeout(Number(process.env.VBOARD_LLM_CHAT_TIMEOUT_MS || 45000))
      });
    } catch (error) {
      const packet = await this.fallback.runRole(args);
      return {
        ...packet,
        provider_error: `remote LLM provider request failed or timed out: ${error.message}`,
        provider_fallback: true
      };
    }
    ...
    if (!response.ok) {
      ...
      return {
        ...packet,
        provider_error: `remote LLM provider ${response.status}: ${text.slice(0, 240)}`,
        provider_fallback: true
      };
    }
    ...
    try {
      ...
      return {
        ...packet,
        provider_error: `remote LLM provider JSON parse failed: ${error.message}; content=${content.slice(0, 220)}`,
        provider_fallback: true
      };
    }
...
module.exports = { RemoteChatProvider };
