...
function stripProviderPrefix(model) {
  return String(model || "").replace(/^together\//, "");
}
...
class TogetherProvider {
  constructor({
    ...
    baseUrl = process.env.TOGETHER_API_BASE || "https://api.together.xyz/v1/chat/completions",
  ...
  liveRoleAllowlist() {
    const configured = process.env.AZZCO_LIVE_ROLE_ALLOWLIST;
    if (configured) {
  ...
  shouldUseExternal({ role, route, workflow, request } = {}) {
    ...
    if (route.restricted && process.env.AZZCO_ALLOW_RESTRICTED_API !== "1") return false;
    if (workflow?.deal_room?.active && role === "deal_captain") {
      return request?.syncPremium === true || process.env.AZZCO_SYNC_DEAL_CAPTAIN === "1";
    }
    ...
    if (["legal_accounting", "invoice_reconciliation"].includes(route.category) && process.env.AZZCO_ALLOW_RESTRICTED_API === "1") {
  ...
  promptFor({ role, route, request, workflow }) {
    return [
      "You are one specialist worker in the AZZCO council.",
  ...
  async runRole(args) {
    ...
    if (!this.shouldUseExternal(args)) {
      ...
      return {
        ...packet,
        provider_note: route.restricted && process.env.AZZCO_ALLOW_RESTRICTED_API !== "1"
    ...
    try {
      response = await fetch(this.baseUrl, {
        ...
        signal: AbortSignal.timeout(Number(process.env.AZZCO_TOGETHER_TIMEOUT_MS || 25000))
      });
    } catch (error) {
      const packet = await this.fallback.runRole(args);
      return {
        ...packet,
        provider_error: `Together request failed or timed out: ${error.message}`,
    ...
    if (!response.ok) {
      ...
      return {
        ...packet,
        provider_error: `Together ${response.status}: ${text.slice(0, 240)}`,
        provider_fallback: true
      };
    }
    ...
    try {
      ...
      return {
        ...packet,
        provider_error: `Together JSON parse failed: ${error.message}; content=${content.slice(0, 220)}`,
        provider_fallback: true
      };
    }
    ...
    const chargedModel = MODEL_REGISTRY[chargedAlias]?.model || `together/${model}`;
