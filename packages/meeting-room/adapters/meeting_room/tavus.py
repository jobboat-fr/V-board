"""Optional Tavus avatar channel for meeting-room advisors.

Tavus has two useful modes for this project:

* full: Tavus runs the complete CVI pipeline for a persona/replica.
* echo: Hermes remains the brain and Tavus renders supplied text/audio.
...
HTTP_TIMEOUT_SECONDS = security.env_int("TAVUS_HTTP_TIMEOUT_SECONDS", 30, minimum=5, maximum=120)


def is_configured(api_key: str | None = None) -> bool:
    return bool(api_key or os.getenv("TAVUS_API_KEY"))


def _api_key(api_key: str | None = None) -> str:
    key = api_key or os.getenv("TAVUS_API_KEY", "")
    if not key:
        raise RuntimeError("TAVUS_API_KEY not set - avatar unavailable")
    return key


def _base_url() -> str:
    raw_url = (os.getenv("TAVUS_BASE_URL") or TAVUS_BASE_URL).rstrip("/")
    parsed = urlparse(raw_url)
    if parsed.scheme != "https" and parsed.hostname not in {"127.0.0.1", "localhost"}:
        raise RuntimeError("Tavus base URL must use https")
    if not parsed.netloc:
        raise RuntimeError("Tavus base URL is invalid")
    return raw_url
...
def advisor_system_prompt(advisor: dict[str, Any], topic: str = "") -> str:
    ...
    return (
        ...
        "When using echo mode, Hermes supplies the actual words; this persona "
    ...
    *,
    ...
    async with httpx.AsyncClient(
        ...
        if not resp.is_success:
            raise _clean_error("Tavus persona", resp.status_code, resp.text, key)
        return resp.json()
    ...
    *,
    ...
    if not persona_id:
        persona = await create_persona(
            api_key=key,
            persona_name=f"Hermes {advisor.get('name', 'Advisor')}",
    ...
    body: dict[str, Any] = {
        ...
        "conversation_name": conversation_name or f"Hermes meeting {security.safe_component(room_id, fallback='room')}",
    ...
    async with httpx.AsyncClient(
        ...
        if not resp.is_success:
            raise _clean_error("Tavus conversation", resp.status_code, resp.text, key)
        data = resp.json()
