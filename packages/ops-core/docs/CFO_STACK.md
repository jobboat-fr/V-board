...
The CFO stack is the deterministic finance/accounting layer for Example Company.
...
All inputs live under `VBOARD_DATA_ROOT/ops/context`:

| File | Required | Meaning |
|---|---:|---|
| `bank_transactions.json` | yes | Normalized bank API/bank transactions. Amounts may be signed, or positive with `side: "debit"` / `side: "credit"`. |
...
Generated under `VBOARD_DATA_ROOT/finance`:
...
- output/context paths attempt to escape `VBOARD_DATA_ROOT`
...
```bash
node src/cli.js finance import-bank path/to/bank_receipt_reconcile.json --trust-bank-api
...
The bank API importer consumes the existing `bank_receipt_reconcile*.json` output from the receipt reconciliation script.
...
By default, imported bank API API snapshots are marked `imported_unverified`, so the CFO build still blocks. To generate a working pack from an API snapshot, explicitly set `trustbank APIApi: true` or pass `--trust-bank-api`.

That flag means: "use this bank API API export as source-of-record for today’s working pack." It does not mean an accountant has validated official bank statements.

Important: the receipt reconciler may generate debit-only reports because receipt matching only needs expenses. Debit-only imports are marked `imported_debit_only` and remain blocked for full CFO reporting, even with `--trust-bank-api`, unless `--allow-debit-only` is explicitly passed for an expenses-only working pack.

For a full finance run, prefer `finance pull-bank`: it pulls both `credit` and `debit` sides from bank API and writes a `bank_api_full_snapshot` validation record.

## Current Findings

- The previous standalone `vboard_finance_engine.js` had useful classification logic, but it was not integrated into the product API/MCP/CLI.
...
- It did not protect finance output paths. The product code now rejects traversal outside `VBOARD_DATA_ROOT`.
