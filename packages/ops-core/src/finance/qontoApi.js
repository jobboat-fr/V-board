...
function qontoAuthFromEnv(env = process.env) {
  if (env.QONTO_AUTH && env.QONTO_AUTH.includes(":")) return env.QONTO_AUTH;
  if (env.QONTO_LOGIN && env.QONTO_SECRET) return `${env.QONTO_LOGIN}:${env.QONTO_SECRET}`;
  return "";
}

function normalizeQontoAmount(transaction) {
...
function normalizeQontoTransaction(transaction, source = {}) {
  ...
  return {
    ...
    amount: normalizeQontoAmount(transaction),
    ...
    sourceKind: "qonto_api_full_snapshot"
  };
}
...
function buildQontoAttachmentMatches(transactions, existingMatches) {
  ...
  for (const tx of transactions) {
    ...
    if (existing) {
      matches.set(key, {
        ...
        source: existing.source || "qonto_existing_attachment"
      });
      continue;
    }
    matches.set(key, {
      ...
      invoicePath: `qonto://attachments/${tx.attachmentIds.join(",")}`,
      source: "qonto_existing_attachment",
      confidence: 1,
      attachmentIds: tx.attachmentIds,
      note: "Qonto transaction already had attachment evidence when pulled from the API."
    });
  }
...
async function fetchJson(fetchImpl, auth, apiBase, pathname, params = {}) {
  ...
  if (!response.ok) {
    const error = new Error(`QONTO_API_${response.status}: ${text.slice(0, 500)}`);
    error.code = "QONTO_API_ERROR";
...
async function fetchTransactionsForAccount(fetchImpl, auth, apiBase, account, since, side) {
  ...
  for (;;) {
    ...
    for (const transaction of data.transactions || []) {
      transactions.push(normalizeQontoTransaction(transaction, {
...
async function pullQontoSnapshot(dataRoot, options = {}) {
  const root = path.resolve(dataRoot);
  const contextDir = resolveUnderRoot(root, options.contextDir || "ops/context");
  const apiBase = options.apiBase || "https://thirdparty.qonto.com/v2";
  const auth = options.auth || qontoAuthFromEnv(options.env || process.env);
  const fetchImpl = options.fetchImpl || globalThis.fetch;
  const since = options.since || "2026-01-01T00:00:00Z";
  if (!auth || !auth.includes(":")) {
    const error = new Error("QONTO_AUTH_MISSING");
    error.code = "QONTO_AUTH_MISSING";
    throw error;
  }
  ...
  if (!accounts.length) {
    const error = new Error("QONTO_BANK_ACCOUNTS_EMPTY");
    error.code = "QONTO_BANK_ACCOUNTS_EMPTY";
    throw error;
  }
  ...
  const invoiceMatches = buildQontoAttachmentMatches(transactions, existingMatches);
  ...
  const validation = {
    ok: true,
    method: "qonto_api_full_snapshot",
    ...
    warning: "Qonto API full snapshot is accepted as source-of-record for a working pack. It is not a signed bank-statement attestation."
  };
...
module.exports = {
  pullQontoSnapshot,
  qontoAuthFromEnv,
  normalizeQontoTransaction,
  extractAttachmentIds,
  buildQontoAttachmentMatches
};
