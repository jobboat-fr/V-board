...
if [ ! -f "$REPO_DIR/docker-compose.yml" ]; then
  ...
  echo "    git clone https://github.com/azzco-labs/v-board.git $REPO_DIR"
  exit 1
fi
...
if [ ! -f "$REPO_DIR/.env" ]; then
  ...
  echo "    AZZCO_COUNCIL_TOKEN"
  echo "    AZZCO_API_TOKEN"
  echo "    AZZCO_OWNER_WHATSAPP"
  echo "    HUGGINGFACE_TOKEN (or TOGETHER_API_KEY)"
fi
